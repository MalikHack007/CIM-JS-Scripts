    ...
    if(answer){
        ...
        setTimeout(()=>{
            ...
            for (let i = 0; i < AISlistElem.children.length; i++){
                if(AISlistElem.children[i].textContent.includes(AISKeyWord) && AISlistElem.children[i].textContent.includes(AISMarkedKeyWord)){
                    handleUndoableTask();
                    return;
                }
            }
